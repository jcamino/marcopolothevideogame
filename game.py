...
class Game(DirectObject):
    ...
    def move_player(self, task):
        ...
        if self.keyMap['left']: 
            self.player.setX(self.player.getX() - dt*20)
            self.player.setR(20)
            planeTurn = self.player.hprInterval(0.1,(180,0,20))
            planeTurn.start()
            ...
            planeTurn = self.player.hprInterval(0.1,(180,0,0))
            planeTurn.start()
    ...
    def update_obstacles(self,task):
        ...
        for obstacle in self.obstacles:
            if obstacle.getName()=="electron":
                dirVec = self.player.getPos()-obstacle.getPos()
                dirVec.normalize()
                obstacle.setPos(obstacle.getPos()+dirVec*dt*10)
            ...
            if obstacle.getY() < -100:
                obstacle.getParent().remove()
            
        
        self.obstacles = [obstacle for obstacle in self.obstacles if obstacle.getY()>-100]
        ...
        for i in range(0,20-len(self.obstacles)):
            ...
            #Finishes setting up collision on new object
            cNode.addSolid(CollisionSphere((0,0,3),3))
            obstacle.attachNewNode(cNode)
            cNodePath = obstacle.attachNewNode(cNode)
...
engine = Game()
>>>>>>> 9e4f4152eca114880a2f4b76fb5fc965c7ed4ad8
run()